...
export async function main(ns) {
    ...
    if (args.dest) {
        ...
        if (args.buySystems) {
            const numSystems = totalSystemsCouldBuy(ns, 64, ns.getServerMoneyAvailable('home'));
            logger.info(`Purchasing ${numSystems} with 64 GB of RAM.`);
            const boughtServers = buyServers(ns, numSystems, 64);
...
export async function startDeployment(ns, logger, server, scriptInfo, metaData, killExisting, target, serversOfInterest) {
    ...
    // Check if it has root access,
        ...
        // Check RAM
        const exitCode = checkRAM(ns, server, scriptInfo.files, metaData);
        if (exitCode) {
