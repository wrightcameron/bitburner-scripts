...
import {
    freeRam, getLargestRamUsage, hackAnalyzeMaxThreads, getScriptRam,
} from '/lib/MemoryProcs.js';
import { totalSystemsCouldBuy, buyServers, handleBuyServers } from '/lib/PurchaseServers.js';
...
export async function main(ns) {
    ...
    // If destination provided, only push script to that system
        if (args.dest) {
            const serversOfInterest = getServersWithBestRates(ns, await getAllKnownServers(ns, logger));
            ...
            const serverList = await getAllKnownServers(ns, logger);
            const serversOfInterest = getServersWithBestRates(ns, serverList);
...
export async function startDeployment(ns, server, scriptInfo, killExisting, target, serversOfInterest) {
    ...
    // Check if it has root access,
        ...
        // Check RAM
        const exitCode = checkRAM(ns, server, scriptInfo.files);
        if (exitCode) {
        ...
        const maxThreads = freeRam(ns, server) / ns.getScriptRam(scriptInfo.entryPoint, server);
...
export async function startScript(ns, host, threads, scriptInfo) {
    ...
    // In this case, it is set to 75% of the server's max money
        ...
        ns.enableLog('ALL');
        const pid = await ns.exec(scriptInfo.entryPoint, host, threads, ...args);
        if (pid !== 0) {
            metaData.pidCount++;
        }
        return pid;
