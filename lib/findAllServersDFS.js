...
export async function getAllKnownServers(ns, logger) {
    const serverList = await getServers(ns, 'home');
    ...
    while (!stack.isEmpty()) {
        ...
        const newServerList = await getServers(ns, server);
...
export async function getAllKnownServersRecursive(ns, seenServers) {
    // TODO This ins't the hardest algorithem to make
    // but I shuold look up the proper way from school first.
    return null;
}
