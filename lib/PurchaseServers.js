...
export function largestRamSizeCouldBuy(ns, money) {
    ...
    for (let i = 1; i <= RAM_CEILING; i = i << 1) {
        let serverCost = ns.getPurchasedServerCost(i << 1);
        if (serverCost >= money) {
            largestRamSize = i;
            break;
        }
...
export function findNumServersWithMaxRam(ns, money) {
    ...
    let currentNumSys = 0;
    for (let i = 1; i <= RAM_CEILING; i = i << 1) {
        let numSystems = totalSystemsCouldBuy(ns, i, money);
...
export function getPurchasedServersMinRam(ns){
    ...
    let serverList = getPurchasedServers();
    for(let server of serverList){
...
export function getBestServersToReplace(ns, maxRam, money) {
    let currentMaxRam = getPurchasedServersMaxRam(ns);
    if (currentMaxRam >= maxRam){
        return []; // There are no best servers to replace
    }
    let numBestServers = totalSystemsCouldBuy(ns, maxRam, money);
    let purchasedServers = getPurchasedServersWithRam(ns);
    let openSlots = ns.getPurchasedServerLimit() - purchasedServers.length;
    if(openSlots >= numBestServers){
        return []; //Still open servers slots
    }
    let toRemove = numBestServers - openSlots;
    let replaceServers = []
    for(let server of purchasedServers){
...
export function handleBuyServers(ns, money){
    ...
    let delServers = getBestServersToReplace(ns, maxRam, money);
    // Remove Servers
    for(let server of delServers){
        ns.killall(server,true)
        ns.deleteServer(server);
    }
    // Buy new servers
    if (0 < ns.getPurchasedServerLimit() - ns.getPurchasedServers().length){
